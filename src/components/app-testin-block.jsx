...
import TestRules from './app-testing-block-rules'
import TestQuestion from './app-testing-block-question'
import TestinBlcokButton from './app-testing-block-button'
import FinishBlock from './app-finish-block';
import engine from '../engine/psyTest'


function AppTestingBlock({progressCapture, userData}) {
    const [rulesTriger, setRulesTriger] = React.useState(true);
    const [finishTriger, setfinishTriger] = React.useState(false);
    const [currentQuestion, setCurrentQuestion] = React.useState(0);
    const [answers, setAnswers] = React.useState([null]);
    const [userDataName, setUserData] = React.useState({});
    ...
    const questions = [
        "Буває, що я серджуся",
        "Зазвичай вранці я прокидаюся свіжим і відпочившим",
        ...
        "Мене вкрай зачіпає, коли мене критикують або лають",
        ...
        "Навряд чи я схочу присвятити все своє життя військовій професії (продовжити контракт, залишитися на службу за контрактом, поступити у військове училище)",
    ...
    const rulesBlockToogle = () => {
        setRulesTriger(false)
    }

    const nextQuestionPlay =()=> {
            setCurrentQuestion(currentQuestion + 1)
            progressCapture(currentQuestion)
    }
    ...
    const checker = (id) => {
        if(currentQuestion < 200){
            ...
            const readyAnswers = engine(answers)
            const newURL = window.location.href;
            console.log(`${newURL}api`)
            axios.post(`${newURL}api`, {
                userDataName,
                readyAnswers
            })
            ...
            setfinishTriger(true)
        }
    }


    return (
        <div className="app-testing-block">

            {rulesTriger && <TestRules rulesTrigger = {rulesBlockToogle}/>}
            {finishTriger && <FinishBlock/>}

            <TestQuestion quest = {questions[currentQuestion]}/>
            <div className="testing-block-buttons">
                <div className="block-btn" data-id="no"
                    onClick = {(evt) => {checker(evt.currentTarget.dataset.id)}}
                >
                    <TestinBlcokButton setup={false}/>
                </div>
                <div className="block-btn" data-id="yes"
                    onClick = {(evt) => {checker(evt.currentTarget.dataset.id)}}
                >
                    <TestinBlcokButton setup={true}/>
                </div>
            </div>
        <div className="answer-block"></div>
        </div>
    );
}

export default AppTestingBlock;
